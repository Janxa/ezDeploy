...
const Dashboard = () => {
  ...
  useEffect(() => {
    WebsiteService.getWebsites().then(
      (response) => {
        ...
        if (!data){
            let dummy_data={website_1:["page1p","apge2"],
            website_2:["page2p","apge2","page3","page2p","apge2","page3","page2p","apge2","page3","page2p","apge2","page3","page2p","apge2","page3",],
  ...
  return (
    <main className="mt-10 md:mt-32 w-full flex flex-col justify-around h-3/5">
        <h2 className="bg-color-bg-dark w-4/5 md:w-1/2 lg:w-1/3 mx-auto p-2 shadow-md rounded-md text-2xl font-bold text-center m-2">Dashboard</h2>
        <div className="bg-color-bg-dark w-4/5 md:w-1/2 lg:w-1/3 mx-auto p-5 shadow-md rounded-md  flex flex-col overflow-hidden">
            <ul className=" text-center overflow-scroll" >
                ...
                Object.keys(websites).map((item,key)=>{
                    return <li key={key} className="flex flex-col border-b border-gray-400 border-dashed mb-4 py-2 ">
                            <p>{item}</p>
                                <div className=" flex w-full justify-around  ">
                                    <button className="button px-2" onClick={()=>showDetails(item)}>Details</button>
